...
import com.modules.util.HQLUtil;
...
public class BaseDaoImpl extends BaseHibernateDaoSupport implements BaseDao {
	...
	public <T> List<T> queryListNotIn(Class<T> entityClass, int pageStart, int pageSize, String orderAttr, String orderType,
			 String notAttr,String notType,Map<String, Object>... attrs) {
		String hql;
		if (ObjValid.isValidMap(attrs)) {
			hql = HQLUtil.createQueryListHQLNotIn(entityClass.getSimpleName(), orderAttr, orderType, notAttr, notType,attrs[0].keySet());
			return this.findPageObjects(hql, pageStart, pageSize, attrs[0].values().toArray());
		} else {
			hql = HQLUtil.createQueryListHQLNotIn(entityClass.getSimpleName(), orderAttr, orderType, notAttr, notType);
			return this.findPageObjects(hql, pageStart, pageSize);
		}
	}
