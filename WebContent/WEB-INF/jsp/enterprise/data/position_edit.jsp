	...
	<div class="main-content">
		<div class="breadcrumbs" id="breadcrumbs">
			...
			<ul class="breadcrumb">
				...
				<li>
					<a href="${ctx}/data/positionAction!retrieveAllPositions.action">人事管理</a>
				</li>
		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-xs-12">
					<!-- PAGE CONTENT BEGINS -->
					<form action="#" method="post" id="positionForm"  class="form-horizontal" role="form">
						<input type="hidden" name="position.id" value="${position.id}"/>
						<input type="hidden" name="position.token" value="${position.token}"/>
						<div class="form-group">
							<label class="col-xs-12 col-sm-1 control-label no-padding-right" for="form-field-1">职位编号</label>
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<input type="text" class="col-xs-10 col-sm-5" id="position.code" name="position.code" value="${position.code}" />
								</div>
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-12 col-sm-1 control-label no-padding-right" for="form-field-1">职位名称</label>
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<input type="text" class="col-xs-10 col-sm-5" name="position.name" value="${position.name}" />
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<textarea name="position.remark" style="width: 385px;height: 100px;" placeholder="最多输入500个字符">${position.remark}</textarea>
								</div>
							</div>
						</div>
		...
		$(document).ready(function(){
			...
			$("#positionForm").validate({
				...
				submitHandler:function(form){
					ajaxUpdate(form,'positionAction!editPosition.action');
			    }
			});
		});
	</script>
