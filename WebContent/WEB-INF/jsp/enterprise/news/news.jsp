	...
	<div class="main-content">
		<div class="breadcrumbs" id="breadcrumbs">
			...
			<ul class="breadcrumb">
				...
				<li>
					<a href="newsAction!retrieveAllNewss.action">父级栏目</a>
				</li>
			...
			<div>
				<h3 class="row-fluid header smaller  blue" style="line-height: 39px;">
					...
					<span class="span5 pull-right">
						<label class="inline">
							<a class="btn btn-success btn-sm" href="newsAction!newPage.action">
								<i class="icon-plus"></i>
								添加
							</a>
						</label>
							...
							<div class="widget-body">	
								<div class="table-responsive">
									...
									<table id="sample-table-1" class="table table-striped table-bordered table-hover dataTable">
										...
										<tbody>
											<c:forEach var="bean" items="${newss}" varStatus="status">
												<tr>
													<td>${status.count}</td>
													<td>${bean.title}</td>
													<td>${bean.content}</td>
													...
													<td>
														<div class="hidden-sm">
															<a class="btn btn-info btn-xs"  title="" href="newsAction!retrieveNewsById.action?id=${bean.id}&pageNo=${pager.pageNo}">修改</a>
									...
									<div class="row">
										<s:form action="newsAction!retrieveAllNewss.action" 
											namespace="/news" method="get" id="form1" name="form1"
												theme="simple">
	...
	<script>
		function doDelete(id) {
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					window.location.href="newsAction!delNews.action?id="+id;
				}
			});
		}
	</script>
