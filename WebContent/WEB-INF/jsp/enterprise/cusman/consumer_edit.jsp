		...
		<div class="page-content">
			<div>
				<h3 class="row-fluid header smaller  blue" style="line-height: 39px;">
					<span class="span7">客户信息编辑 </span><!-- /span -->
			...
			<div class="row">
				<div class="col-xs-12">
					<!-- PAGE CONTENT BEGINS -->
					<form action="#" method="post" id="consumerForm"  class="form-horizontal" role="form">
						<input type="hidden" name="consumer.id" value="${consumer.id}"/>
						<input type="hidden" name="consumer.token" value="${consumer.token}"/>
						<div class="form-group">
							<label class="col-xs-12 col-sm-1 control-label no-padding-right" for="form-field-1">客户名称</label>
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<input type="text" class="col-xs-10 col-sm-5" name="consumer.name" id="consumer.name" value="${consumer.name }"/>
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<input type="text" class="col-xs-10 col-sm-5" name="consumer.phone" value="${consumer.phone }"/>
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<input type="text" class="col-xs-10 col-sm-5" name="consumer.address" value="${consumer.address }"/>
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<select name="industryId" class="form-control" id="form-field-select-1" style="width: 250px">
										<option value="${consumer.industryId.id }">${consumer.industryId.name }</option>
										<c:forEach items="${consumerindustry }" var="industry">
											<c:if test="${consumer.industryId.id ne industry.id}">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<select name="typeId" class="form-control" id="form-field-select-1" style="width: 250px">
										<option value="${consumer.typeId.id }">${consumer.typeId.name }</option>
										<c:forEach items="${consumerType }" var="type">
											<c:if test="${consumer.typeId.id ne type.id}">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<select name="levelId" class="form-control" id="form-field-select-1" style="width: 250px">
										<option value="${consumer.levelId.id }">${consumer.levelId.name }</option>
										<c:forEach items="${consumerLevel }" var="level">
											<c:if test="${consumer.levelId.id ne level.id}">
						...
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<textarea name="consumer.description" style="width: 385px;height: 100px;" placeholder="最多输入500个字符">${consumer.description }</textarea>
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<input type="text" class="col-xs-10 col-sm-5" name="consumer.fax" value="${consumer.fax }"/>
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<input type="text" class="col-xs-10 col-sm-5" name="consumer.postCode" value="${consumer.postCode }"/>
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<input type="text" class="col-xs-10 col-sm-5" name="consumer.revenue" value="${consumer.revenue }"/>
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<textarea name="consumer.remark" style="width: 385px;height: 100px;" placeholder="最多输入500个字符">${consumer.remark }</textarea>
								</div>
							</div>
						</div>
						<div class="form-group">
		...
		$(document).ready(function(){
			...
			$("#consumerForm").validate({
				...
				submitHandler:function(form){
					 ajaxUpdate(form,'consumerAction!editConsumer.action');
			    }
			});
		});
	</script>
