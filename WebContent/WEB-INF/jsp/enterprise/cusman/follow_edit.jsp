	...
	<div class="main-content">
		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-xs-12">
					<!-- PAGE CONTENT BEGINS -->
					<form action="followAction!editFollow.action" method="post" id="followForm"  class="form-horizontal" role="form">
						<input type="hidden" name="follow.id" value="${follow.id}"/>
						<input type="hidden" name="follow.token" value="${follow.token}"/>
								...
								<div class="clearfix">
									<select name="consumerId" class="width-80 chosen-select" id="form-field-select-1" style="width: 250px">
										<c:forEach items="${consumers }" var="consumer">
											<option value="${consumer.id }" <c:if test="${follow.consumerId.id == consumer.id }">selected="selected"</c:if> >
												${consumer.name }
											</option>
										</c:forEach>
									</select>
								...
								<div class="clearfix">
									<select name="typeId" class="form-control" id="form-field-select-1" style="width: 250px">
										<c:forEach items="${types }" var="type">
											<option value="${type.id }" <c:if test="${follow.type.id == type.id}">selected="selected"</c:if>>${type.name }</option>
										</c:forEach>
									</select>
								</div>
							</div>
						...
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="input-group date form_datetime col-md-4" data-date-format="yyyy-mm-dd HH:mm:ss" data-link-field="followTime">
				                    <input class="form-control" style="margin-left: -13px;" size="16" type="text" value="<fmt:formatDate value="${follow.followTime}" pattern="yyyy-mm-dd HH:mm:ss"/>" readonly>
				                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
									<span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span>
				                </div>
								<input type="hidden" id="followTime" name ="follow.followTime" value="<fmt:formatDate value="${follow.followTime}" pattern="yyyy-MM-dd HH:mm:ss"/>" /><br/>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<textarea name="follow.content" style="width: 385px;height: 100px;" placeholder="最多输入500个字符">${follow.content }</textarea>
								</div>
							</div>
						</div>
		...
		$(document).ready(function(){
			...
			$("#followForm").validate({
				...
				submitHandler:function(form){
					 ajaxUpdate(form,'followAction!editFollow.action');
			    }
			});
