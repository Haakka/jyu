			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form action="#" method="post" id="orderForm"  class="form-horizontal" role="form">
						...
						<input type="hidden" name="order.token" value="${order.token }"/>
						
						<div class="form-group">
							<label class="col-xs-12 col-sm-1 control-label no-padding-right" for="form-field-1">订单号</label>
							<div class="col-xs-12 col-sm-10">
								<div>
									<input type="text" name="orderSn" value="${order.orderSn }" disabled="disabled" style="width: 250px;"/>
								</div>
							</div>
						</div>
						<div class="form-group">
								...
								<div>
									<select name="contractId" style="width:300px;" class="width-80 chosen-select" id="form-field-select-3" data-placeholder="请选择合同...">
										...
										<c:forEach items="${contracts }" var="contract">
											<option value="${contract.id }" <c:if test="${contract.id == order.contractId.id }">selected="selected"</c:if> >${contract.num }</option>
										</c:forEach>
									</select>
								</div>
							</div>
								...
								<div class="clearfix">
									<select name="finisherId" class="width-80 chosen-select" id="form-field-select-3" data-placeholder="请选择完成者...">
										...
										<c:forEach items="${finishers }" var="finisher">
											<option value="${finisher.id }" <c:if test="${finisher.id == order.finisher.id }">selected="selected"</c:if>>${finisher.name }</option>
										</c:forEach>
									</select>
								</div>
							</div>
						...
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="clearfix">
									<input type="text" class="col-xs-10 col-sm-5" name="order.totalAmount" style="width:200px;" value="${order.totalAmount }"/>
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-xs-12 col-sm-10">
								<div class="input-group">
									<input class="form-control date-picker" name="order.payTime" id="id-date-picker-1" type="text" data-date-format="yyyy-mm-dd" style="width: 261px;" value="<fmt:formatDate value="${order.payTime}" pattern="yyyy-MM-dd"/>"/>
								...
								<div class="clearfix">
									<select name="payTypeId"  id="form-field-select-2">
										<c:forEach items="${payTypes }" var="payType">
											<option value="${payType.id }" <c:if test="${payType.id == order.payType.id }">selected="selected"</c:if>>${payType.name }</option>
										</c:forEach>
									</select>
								</div>
							</div>
							...
							<div class="col-xs-12 col-sm-10">
								<select name="productId" style="width:300px;" class="width-80 chosen-select" id="form-field-select-3" data-placeholder="请选择合同...">
										...
										<c:forEach items="${products }" var="product">
											<option value="${product.id }" <c:if test="${product.id == order.productId.id }">selected="selected"</c:if>>${product.name }</option>
										</c:forEach>
									</select>
							</div>
						</div>
	...
	<script>
		...
		$(document).ready(function(){
				...
				submitHandler:function(form){
					ajaxUpdate(form,'orderAction!editOrder.action');
			    }
